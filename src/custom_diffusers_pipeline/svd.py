...
class StableVideoDiffusionPipeline(DiffusionPipeline):
    ...
    def __init__(
        ...
        unet: UNetSpatioTemporalConditionModel,
        scheduler: EulerDiscreteScheduler | CustomEulerDiscreteScheduler,
    ...
    def decode_latents(self, latents, num_frames, decode_chunk_size=14):
        ...
        accepts_num_frames = "num_frames" in set(
            inspect.signature(self.vae.forward).parameters.keys()
        )
    ...
    def __call__(
        ...
        for i in range(len(temp_cond)):
            temp_cond_ = self.image_processor.preprocess(
                temp_cond[i], height=height, width=width
            )
        ...
        for i in range(temp_cond.shape[0]):
            temp_cond_latents_ = self._encode_vae_image(
                temp_cond[i : i + 1, :, :, :],
                device,
                num_videos_per_prompt,
                do_classifier_free_guidance,
            )  # [12, 4, 72, 128]
            temp_cond_latents_ = rearrange(
                temp_cond_latents_, "(b f) c h w -> b f c h w", b=2
            )
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                latents = self.scheduler.step_single_dgs(
                    noise_pred, t, latents, temp_cond_latents, mask, lambda_ts, step_i=i
                ).prev_sample
